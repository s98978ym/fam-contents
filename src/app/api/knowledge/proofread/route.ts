...
(b) 筆者の文体の特徴を「そのまま」把握する。変換や解釈はしない。
  - 実際に使われている語尾をそのまま抽出する（例: 「〜してください」「〜を登録」「〜する」）
  - 文の構造（箇条書き中心か、文章中心か、混在か）
  - テンション（淡々と事実を並べている、熱量がある、等）
  - 句読点・記号の使い方（「！」「？」を使っているか、使っていないか）
発展テキストでは、ここで把握した文体をそのまま維持する。
元テキストに存在しない語尾・言い回し・テンションを足さない。
...
- 元テキストの文体をそのまま維持する。テンションを上げも下げもしない
  - 元が「〜してください」「〜を登録」のような淡々とした箇条書きなら、追加部分も同じトーンで淡々と書く
  - 元が「！」や「？」を使っていないなら、追加部分でも使わない
  - 元に存在しないカジュアル表現（「めっちゃ」「ポチッと」「〜じゃない？」等）を足さない
  - 元に存在しない丁寧表現（「〜いたします」「〜でございます」等）も足さない
...
原則: 元テキストに存在する表現の範囲内で書く。存在しない表現を足さない。

具体例で示す（元テキストが淡々とした箇条書きの場合）:
○「議事録の形式をAIに学習させると、整理の精度が上がる」（元と同じ淡々としたトーン）
×「これ、めっちゃ便利じゃない？」（元にないカジュアル表現を追加している）
×「ポチッと押すと...」（元にない擬音・演出表現を追加している）
×「底上げされるはず！」（元にないテンション・「！」を追加している）

絶対に使わない:
- × 元テキストに存在しないテンションの語尾（「〜じゃない？」「〜だよね」「〜かな？」等。元テキストにあれば可）
- × 元テキストに存在しない感嘆符「！」や擬音語（元テキストにあれば可）
- ×「感情的な繋がり」「多角的なアプローチ」「包括的な戦略」のようなAI特有の抽象語
- ×「これは非常に重要な観点です。なぜなら〜」のような冗長な前置き
- × 元テキストにない硬い表現への言い換え（「実施する」「活用する」「推進する」等）
