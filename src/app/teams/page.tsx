...
export default function TeamsPage() {
  const { teams, addTeam, updateTeam, deleteTeam, addMember, removeMember, currentTeamId, setCurrentTeamId } = useTeam();
    ...
    <div className="max-w-2xl">
      ...
      {teams.length === 0 && !showNewForm ? (
        <div className="bg-white rounded-xl border-2 border-dashed border-slate-200 p-8 text-center">
          ...
          <p className="text-sm text-slate-500 mb-1">チームがまだありません</p>
        ...
        <div className="space-y-4">
          {teams.map((team) => {
            ...
            return (
              <div key={team.id} className={`bg-white rounded-xl border shadow-sm overflow-hidden ${isActive ? "border-indigo-300 ring-1 ring-indigo-100" : "border-slate-200"}`}>
                {/* Header */}
                  ...
                  {isEditing ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => { if (confirm(`「${team.name}」を削除しますか？\nメンバー情報も削除されます。`)) { deleteTeam(team.id); showToast("削除しました"); } }} className="p-1.5 rounded-md hover:bg-red-50 text-slate-400 hover:text-red-600 transition-colors" title="削除">
