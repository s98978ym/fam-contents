...
| サーバー専用モジュールのモジュールレベル副作用でVercelクライアントエラー | Next.jsバンドラはサーバー専用ファイル（`src/lib/xxx.ts`）もクライアントバンドルに含めることがある。モジュールのトップレベルで `console.log()` 等を即時実行すると、その時点でNode.js専用API（`crypto` 等）が評価され、クライアント側で `ReferenceError` や `Module not found` が発生する。「サーバーサイド専用」とコメントしてもバンドラは無視する | **モジュールレベルの副作用を排除し、server-onlyパッケージを使用する**: (1) `npm install server-only` でパッケージをインストール (2) サーバー専用モジュールの先頭で `import "server-only"` を追加 (3) トップレベルで `console.log()` 等を即時実行しない (4) ログ出力は関数内で遅延実行（lazy initialization）にする |
| APIレスポンス形式変更時にクライアント側が未更新で `e.map is not a function` | `source` フィールドを追加するためにAPIレスポンスを配列 `[...]` からオブジェクト `{ items: [...], source: "..." }` に変更したが、クライアント側のコードは配列を期待したまま `.map()` を呼び出していた。APIルートとクライアントコードの変更が同期されていなかった | **APIレスポンス形式を変更する際のチェックリスト**: (1) **全ての呼び出し元を検索**: `Grep` で該当APIエンドポイントを呼び出している全ファイルを特定 (2) **クライアント側を同時に修正**: API変更とクライアント変更は必ず同一コミットで行う (3) **配列→オブジェクトラッピング時の定型パターン**: 配列を返していたAPIに `source` を追加する場合、クライアント側は `data` → `data.items \|\| []` のように修正。(4) **TypeScript型定義を活用**: APIレスポンス型を定義し、クライアント側でも同じ型を参照することで不整合を検出しやすくする |
| 技術的選択肢を提示せずに実装を進めた | Google OAuth実装時、Code Flow（サーバー側でトークン交換）を「ベストプラクティス」として選択したが、redirect_uri設定でエラー。Token Flow（クライアント完結）の方がこのプロジェクトに適していた。選択肢を提示せず勝手に決めたため、手戻りが発生 | **技術的選択肢がある場合は必ず提示する**: (1) 選択肢A/Bのメリット・デメリットを表形式で示す (2) このプロジェクトの制約（DB有無、クライアント/サーバー中心など）を踏まえた推奨を添える (3) 最終判断はユーザーに委ねる。「ベストプラクティス」を機械的に適用せず、プロジェクト固有の制約を考慮する |
| バグを「期待される動作」として弁護した | Google Pickerでフォルダ内のファイルが表示されないバグを報告されたとき、「Pickerはフォルダ選択用なのでファイルは表示されません」と誤った説明をした。実際は `setMimeTypes("application/vnd.google-apps.folder")` という自分のコードが原因だった。バグを調査せず、仕様だと決めつけて弁護した | **ユーザーから「動かない」と報告されたら、まず自分のコードを疑う**: (1) 「仕様です」「期待される動作です」と即答しない (2) 実装コードを読み直し、フィルタ・制限・条件分岐を確認 (3) API/ライブラリのドキュメントで本来の動作を確認 (4) 原因が特定できない場合は「調査します」と回答し、推測で弁護しない。自分が書いたコードのバグである可能性を常に念頭に置く |
...
- **配列を返すAPIは必ずオブジェクトでラップする**: `source` フィールドを含めるため、生の配列 `[...]` ではなく `{ items: [...], source: "..." }` 形式で返す
- **レスポンス形式を変更したら全クライアントを同時更新**: `Grep` でAPIエンドポイントの呼び出し元を検索し、同一PRで修正する
- **クライアント側の定型パターン**: `data.items || []` のように、プロパティアクセス + フォールバックを組み合わせる
- **既存APIのレスポンス形式一覧**:
  - `/api/drive/folders` GET → `{ folders: [...], source }`
  - `/api/drive/files` GET → `{ files: [...], categorized: {...}, source }`
  - `/api/prompt-versions` GET → `[...]`（配列直接）
  - `/api/campaigns`, `/api/contents`, etc. → `[...]`（配列直接、将来的にラップ推奨）
...
7. [ ] 「実装完了の定義」の6項目を全て確認
8. [ ] **APIレスポンス形式変更時は全クライアントを同時更新**（`Grep` で呼び出し元を検索）
...
- **サーバーサイド専用**: `crypto` モジュールを使用するため `import "server-only"` が必須。クライアントサイドからのインポートを防止
