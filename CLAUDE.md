...
- **必須: `fallback_reason` フィールドも含める**。シミュレーション時は理由（「GEMINI_API_KEYが未設定」「API呼び出し失敗」等）を返す
- **必須: UIでフォールバック動作を明示する**。シミュレーション時は警告バナー（amber）、Gemini成功時は「Gemini」バッジ（green）
- **必須: 新規AI機能追加時のチェックリスト** — AI処理を含む新機能を作る際は以下を全て実装してからPRする:
  1. APIレスポンスに `source` + `fallback_reason` を含める
  2. UIにsource表示（Geminiバッジ / シミュレーション警告）を表示する
  3. エラー・ネットワーク障害時のcatchブロックでもsourceを設定する
  4. ローディングUI・進行演出がある場合、完了後にsource表示を必ず隣接させる
...
- **文体の指定は具体的なロールで**: 「プレーンテキストで出力」ではなく「チームの先輩が口頭で教えるような文体」のように指定する
